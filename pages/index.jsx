"use client";
import Head from "next/head";
import { useEffect, useState } from "react";
import { IoClose } from "react-icons/io5";
import bgImg from "../public/jpng.png";
export default function Home() {
  const [isOpen, setIsOpen] = useState(false);

  let data = [
    ["تب", "تومور"],
    ["ضعف جسمانی", "هایپر پلازی", "سنگ کلیه", "تومور"],
    ["خلط", "سنگ کلیه", "rgrg"],
    ["ضعب عضله", "ایدز", "تومور"],
    ["تب", "تومور"],
    ["ضعف جسمانی", "هایپر پلازی", "سنگ کلیه", "تومور"],
    ["خلط", "سنگ کلیه", "rgrg"],
    ["ضعب عضله", "ایدز", "تومور"],
    ["تب", "تومور"],
    ["ضعف جسمانی", "هایپر پلازی", "سنگ کلیه", "تومور"],
    ["خلط", "سنگ کلیه", "rgrg"],
    ["ضعب عضله", "ایدز", "تومور"],
    ["تب", "تومور"],
    ["ضعف جسمانی", "هایپر پلازی", "سنگ کلیه", "تومور"],
    ["خلط", "سنگ کلیه", "rgrg"],
    ["ضعب عضله", "ایدز", "تومور"],
  ];
  const [showDiseasesItem, setShowDiseasesItem] = useState(false);
  const [searchValue, setSearchValue] = useState("");
  const [selectedSymptom, setSelectedSymptomymptom] = useState([]);
  const [selectedDiseaes, setSelectedDiseases] = useState([]);
  const [diseasesSymptom, setDiseasesSymptom] = useState([]);
  useEffect(() => {
    setSelectedDiseases([
      selectedSymptom.map((symptom) => symptom.filter((element, index) => index > 0)),
    ]);
  }, [selectedSymptom]);
  useEffect(() => {
    setDiseasesSymptom(
      [...new Set(selectedDiseaes.flat(2))].map((value) => ({
        value,
        length: selectedDiseaes.flat(2).filter((diseaes) => diseaes === value).length,
      }))
    );
  }, [selectedDiseaes]);
  return (
    <div>
      <Head>
        <title>Medical App</title>
        <link rel="icon" href="data:image/png;base64," />
        <link href="https://v1.fontapi.ir/css/Vazir" rel="stylesheet"></link>
      </Head>
      <main>
        <div
          dir="rtl"
          className="bg-[#efefef] flex min-h-screen w-full overflow-hidden  flex-col items-center  p-24 "
        >
          <div className="absolute z-0 rotate-[] -top-20 max-w-full overflow-hidden ">
            <img
              src="https://www.uplooder.net/img/image/76/827af7b6222f2c00a025def1b7175d33/jpng.png"
              alt=""
              className=" min-w-full "
            />
          </div>
          <div className="rounded-t-3xl bg-[#fefefe] z-[5] backdrop-blur-md  bg-opacity-30">
            <div className="w-[70vw] max-w-[550px] mt-3 mx-3 flex flex-wrap gap-2">
              {selectedSymptom.map((symptom) => (
                <div
                  className={`flex bg-[#2079ff] justify-between text-[#ffff] font-bold border rounded-2xl text-[15px]   border-[#0067ff] `}
                >
                  <p className="self-center px-3 py-1"> 
                  {symptom[0]}{" "}

                  </p>
                  <div
                    onClick={() => {
                      setSelectedSymptomymptom(
                        selectedSymptom.filter(
                          (element, index) => element !== symptom
                        )
                      );
                    }}
                    className="p-1 text-white font-semibold self-center"
                  >
                    <IoClose className="self-center cursor-pointer h-[30px] w-[30px] p-[5px] hover:text-red-600 hover:p-[2px] hover:pr-1 duration-200"/>
                  </div>{" "}
                </div>
              ))}
            </div>
            <div className=" px-5 pt-5">
              <input
                className="h-16 px-5 rounded-3xl w-full outline-[#0067ff]"
                onClick={() => {
                  setIsOpen(!isOpen);
                }}
                placeholder=" علائم را وارد کنید ..."
                id="5"
                type="text"
                value={searchValue}
                onChange={(e) => setSearchValue(e.target.value)}
              />
              <div
                className={`flex flex-col origin-top overflow-y-auto max-h-[180px]   duration-300 `}
                onClick={() => {
                  setIsOpen(true);
                }}
              >
                {data.map(
                  (data) =>
                    data[0].includes(searchValue) && (
                      <div
                        className={`flex flex-col origin-top bg-white bg-opacity-[36%] backdrop-blur-sm py-[12px] rounded-2xl px-5 cursor-pointer  my-[2px] ${
                          selectedSymptom.flat().find((e) => e === data[0]) &&
                          "hidden"
                        } ${!isOpen && "scale-y-0 "} duration-300 `}
                        onClick={() => {
                          setSelectedSymptomymptom([...selectedSymptom, data]);
                          setSearchValue("");
                          setIsOpen(false);
                        }}
                      >
                        {data[0]}{" "}
                      </div>
                    )
                )}
              </div>
            </div>
          </div>
          <div
            onClick={() => {
              setShowDiseasesItem(!showDiseasesItem);
            }}
            className="bg-[#fefefe] z-[5] backdrop-blur-md  bg-opacity-25 w-[70vw] max-w-[550px] text-center z-10"
          >
            <button className="bg-[#0067ff] text-lg w-[60vw] max-w-[470px] text-center text-white font-semibold rounded-2xl p-3">
              تشخیص!
            </button>
          </div>
          <div
            className={` ${
              !showDiseasesItem && "hidden"
            } z-10 bg-white backdrop-blur-md  bg-opacity-25 w-[70vw] max-w-[550px] px-4 text-center py-5 rounded-b-3xl `}
          >
            {diseasesSymptom.sort((a, b) => b.length - a.length).map((e) => (
              <div
                className={`flex  bg-white bg-opacity-80 px-3  my-2  flex-col rounded-xl `}
              >
                <div className="flex justify-between py-5">
                  <div>{e.value}</div>
                  <div>eeeee</div>
                </div>
                <div className=" flex flex-wrap pb-5 gap-2 rounded-xl">
                  {selectedSymptom.map((ww) =>
                    ww.map(
                      (ee) =>
                        ee === e.value && (
                          <div className="bg-[#edf8f7]  text- border rounded-2xl text-[15px] px-3 py-1 border-[#0067ff]">
                            {ww[0]}
                          </div>
                        )
                    )
                  )}
                </div>{" "}
              </div>
            ))}
          </div>
        </div>
      </main>
    </div>
  );
}
